...
function Experience() {
 ...
 // change the toggle to display or hide initailized examples
  function changeTog() {
    ...
    setDisplayNewwForm(false)
  }


  // handle input changes in the initialized experience inputs
  function handleExperienceInputChange(index, value) {
    const newExperienceList = [...experienceInput];
    newExperienceList[index].name = value;
    setExperinceInput(newExperienceList);
  }
...
function saveExperienceForm() {
    setAllData((prevData) => ({
      ...prevData,
      experience: [...experienceInput, { name: newFormmInput }],
    }));
    setDisplayNewwForm(false);
  ...
  return (
    ...
    <div className="flex flex-col">
      ...
      {displayNewwForm && (
        <div id="show-experience" className="w-96 mx-auto bg-white p-3 block rounded-b-md">
          <form>
            <label className="font-medium" htmlFor="company-name">Company Name</label>
            <input
              id="company-name"
              ...
              onChange={(e) => handleExperienceFormInputChange(e, 'companyName')}
            />
            <label className="font-medium" htmlFor="position-title">Position Title</label>
            <input
              id="position-title"
              ...
              onChange={(e) => handleExperienceFormInputChange(e, 'positionTitle')}
            />
            <label className="font-medium" htmlFor="date-started">Start Date</label>
            <input
              id="date-started"
              placeholder="Enter Company Name"
              className="block w-full bg-gray-100 border-2 h-[40px] text-[14px] my-2 px-4 rounded-xl focus:border-blue-600 focus:outline-none"
              ...
              onChange={(e) => handleExperienceFormInputChange(e, 'startDate')}
            />
            <label className="font-medium" htmlFor="date-ended">End Date</label>
            <input
              id="date-ended"
              placeholder="Enter Company Name"
              className="block w-full bg-gray-100 border-2 h-[40px] text-[14px] my-2 px-4 rounded-xl focus:border-blue-600 focus:outline-none"
            ...
            />
            <label className="font-medium my-2" htmlFor="description">Description</label>
            <textarea
              ...
              onChange={(e) => handleExperienceFormInputChange(e, 'description')}
            >
              Enter Description
            </textarea>
            <div className="flex justify-between mt-3">
              <div>
                <button className="flex bg-white border-2 border-slate-400 gap-1 rounded items-center p-1 text-gray-800" onClick={handleDelete}>
                <MdDelete />
                  Delete
                </button>
              </div>
              <div className="flex gap-2">
                <button className="bg-white border-2 border-slate-400 rounded px-2 text-gray-800" onClick={handleCancel}>
                  Cancel
                </button>
                <button className="bg-blue-600 text-white p-1 px-2 rounded" onClick={saveExperienceForm}>
                  Save
                </button>
              </div>
            </div>
